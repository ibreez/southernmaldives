...
import { Resend } from 'resend'; // 1. Swap Nodemailer for Resend
import { query } from './db.js';

dotenv.config();

// 2. Simplify your configuration to use an API Key instead of SMTP details
const emailConfig = {
...
// 4. Update the verification step (or remove it, as APIs don't use 'verify')
export async function verifySmtpConnection() {
  if (!emailConfig.apiKey) throw new Error("Resend API key is missing");
  return true; 
}
...
// 5. Rewrite sendEmail to use the Resend API
export async function sendEmail({ to, subject, html, replyTo }) {
  ...
  const { data, error } = await resend.emails.send({
    ...
    to: [to], // Resend expects an array or comma-separated string
    subject: subject,
    html: html,
    reply_to: replyTo,
  });
