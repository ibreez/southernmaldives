    ...
    <>
      ...
      <Dialog open={isOpen} onOpenChange={onClose}>
        {/* Mobile: Full-screen height with rounded-t only. Tablet/Desktop: Centered dialog */}
         <DialogContent className="scrollbar-hide w-full sm:max-w-4xl h-[100dvh] sm:h-auto max-h-[100dvh] sm:max-h-[90vh] overflow-y-auto p-0 border-none bg-white shadow-2xl rounded-none sm:rounded-[2.5rem] mt-16 sm:mt-12 flex flex-col">
                        ...
                        <div className="relative overflow-hidden rounded-[2.5rem] bg-white border border-slate-100 shadow-xl">
                          ...
                          <div className="h-48 sm:h-56 relative group">
                            <div 
                              ...
                              style={{ 
                                backgroundImage: `url(${
                                  hotels.find(h => h.name === formData.destination)?.images?.[0] || 
              ...
              {currentStep === 4 && (
                <motion.div key="s4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <button type="button" onClick={() => handleInputChange('airport_transfer', !formData.airport_transfer)}
                      className={cn("flex items-center gap-3 sm:gap-4 p-3 sm:p-4 rounded-3xl border transition-all text-left", 
                        formData.airport_transfer ? "border-[#022c22] bg-[#022c22]/5" : "border-slate-100 bg-white")}>
                      <div className={cn("w-10 h-10 rounded-xl flex items-center justify-center shrink-0", formData.airport_transfer ? "bg-[#022c22] text-white" : "bg-slate-50 text-slate-400")}><Plane className="w-5 h-5" /></div>
                      <div className="flex-1">
                        <div className="text-[9px] sm:text-[10px] uppercase tracking-[0.1em] font-bold">Transfer</div>
                        <div className="text-[8px] sm:text-[9px] text-slate-400 italic">Speedboat escort</div>
                      </div>
                      <div className={cn("w-5 h-5 rounded-full border flex items-center justify-center shrink-0", formData.airport_transfer ? "bg-[#022c22] border-[#022c22]" : "border-slate-100")}>{formData.airport_transfer && <Check className="w-3 h-3 text-white" />}</div>
                    </button>
                    <div className="grid grid-cols-2 gap-2">
                        {mealPlans.map((m) => (
                          <button key={m.value} type="button" onClick={() => handleInputChange('meal_plan', m.value)} 
                            className={cn("py-2 px-2 border rounded-xl text-[8px] sm:text-[9px] uppercase tracking-widest font-bold transition-all", 
                              formData.meal_plan === m.value ? "bg-[#022c22] text-white" : "bg-white text-slate-400 border-slate-100")}>{m.label}</button>
                        ))}
                    </div>
                  </div>
                  <Textarea placeholder="Personal Requests (e.g. dietary needs)..." value={formData.special_requests} onChange={(e) => handleInputChange('special_requests', e.target.value)} 
                    className="min-h-[100px] rounded-[1.5rem] bg-slate-50/20 pt-4 px-6 italic text-[13px]" />
                </motion.div>
              )}
                ...
                <motion.div key="s5" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6 max-w-lg mx-auto">
                  ...
                  <div className="flex justify-center gap-2 sm:gap-3">
                    {contactMethods.map((c) => {
                      ...
                      return (
                        <button key={c.value} type="button" onClick={() => handleInputChange('contact_preference', c.value)} 
                          className={cn("flex-1 p-3 rounded-[1.2rem] border flex flex-col items-center gap-2 transition-all", 
                            isActive ? "border-[#022c22] bg-emerald-50/50" : "border-slate-100 bg-white opacity-40")}>
                          <Icon className={cn("w-4 h-4 sm:w-5 sm:h-5", isActive ? "text-[#022c22]" : "text-slate-300")} />
                          <span className="text-[7px] sm:text-[8px] font-bold tracking-[0.1em] uppercase">{c.label}</span>
                        </button>
                      );
                    })}
                  </div>
                  <div className="grid grid-cols-1 gap-4 sm:gap-6 mt-4">
                    {['name', 'email', 'phone'].map((field) => (
                      <Input key={field} placeholder={field.toUpperCase()} value={formData[field as keyof FormData] as string} onChange={(e) => handleInputChange(field as keyof FormData, e.target.value)} 
                        className="border-0 border-b border-slate-200 rounded-none h-10 px-0 bg-transparent text-lg font-serif italic focus-visible:ring-0 focus-visible:border-[#022c22]" />
                    ))}
                  </div>
                </motion.div>
              )}
